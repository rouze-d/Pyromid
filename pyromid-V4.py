...
def PrivateKeyFromMnemonic(Magic):
    for i in range(0, 1):
        ...
        #bAddrT = encode('bc', 0, bytePrivate)

        public_key1 = bit.PrivateKey(wifCompressed).public_key
        public_key1 = ripemd160_sha256(public_key1)
        bAddrW = encode('bc', 0, public_key1)

        #public_key2 = bit.PrivateKey(wifUnCompressed).public_key
        #public_key2 = ripemd160_sha256(public_key2)
        #bAddrT = encode('bc', 0, public_key2)



        return compressedAddr, uncompressedAddr, sAddr, bAddrW, Private_Hex
...
while True:
    z += threadCount
    wod = GetMnemonic()
    compressAddress, UncompressAddress, sAddr, bAddrW, PrivateKey = PrivateKeyFromMnemonic(wod)
    ...
    if compressAddress in richList or UncompressAddress in richList or sAddr in richList or bAddrW in richList:
        w += 1
        ...
        print(f"{ln * 30}\n"
              ...
              f"[{cyan}{z}{reset}] {yellow}P2SH          : {reset}{sAddr}\n"
              f"[{cyan}{z}{reset}] {yellow}Bech32/Segwit : {reset}{bAddrW}\n"
              #f"[{cyan}{z}{reset}] {yellow}Bech32T      : {reset}{bAddrT}\n"
